...
const showFinishedMatches = shallowRef(false)
const finishedMatchesCount = computed(() => data.matches.value.filter((match) => match.status === 'FINISHED').length)
const visibleMatches = computed(() => data.matches.value.filter((match) => {
  const matchesStatus = status.value === 'ALL' || match.status === status.value
  const matchesQuery = `${match.home_team} ${match.away_team} ${match.stage}`.toLowerCase().includes(query.value.toLowerCase())
  const finishedMatchIsVisible = match.status !== 'FINISHED' || status.value === 'FINISHED' || showFinishedMatches.value

  return matchesStatus && matchesQuery && finishedMatchIsVisible
}))
onMounted(data.load)
</script>
...
<template>
  <main class="mx-auto max-w-7xl space-y-6 px-3 py-5 sm:px-6">
    ...
    <template v-else-if="data.campaign.value"><CampaignSettings :campaign="data.campaign.value" :saving="data.busyId.value === 'Configuración guardada'" @save="data.saveCampaign" /><div class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px]"><section><div class="mb-4 flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 sm:flex-row"><label class="sr-only" for="match-search">Buscar partido</label><input id="match-search" v-model="query" class="min-h-11 flex-1 rounded-xl border border-slate-300 px-3" placeholder="Buscar selección o fase" /><select v-model="status" aria-label="Filtrar por estado" class="min-h-11 rounded-xl border border-slate-300 px-3"><option value="ALL">Todos</option><option value="SCHEDULED">Programados</option><option value="LIVE">En vivo</option><option value="FINISHED">Finalizados</option></select><button v-if="status === 'ALL' && finishedMatchesCount" type="button" class="min-h-11 rounded-xl border border-slate-300 px-4 text-sm font-semibold text-slate-700 transition hover:border-brasper-indigoStrong/40 hover:text-brasper-indigoStrong focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brasper-indigoStrong/40" :aria-expanded="showFinishedMatches" @click="showFinishedMatches = !showFinishedMatches">{{ showFinishedMatches ? 'Ocultar finalizados' : `Mostrar finalizados (${finishedMatchesCount})` }}</button></div><div class="grid items-start gap-4 md:grid-cols-2"><MatchCard v-for="match in visibleMatches" :key="match.id" :match="match" :campaign-defaults="data.campaign.value" :busy="data.busyId.value === match.id || (match.coupon_id != null && data.busyId.value === match.coupon_id)" @select="data.select" @action="data.couponAction" /></div><p v-if="!visibleMatches.length" class="rounded-2xl border border-dashed border-slate-300 p-8 text-center text-slate-500">No hay partidos para este filtro. Sincroniza el calendario o cambia la búsqueda.</p></section><NotificationInbox :notifications="data.notifications.value" :busy-id="data.busyId.value" @read="data.readNotification" /></div></template>
  </main>
</template>
