...
async function saveCommission(id: string): Promise<void> {
  const current = comisionesStore.commissions.find((item) => item.id === id)
  if (!current) {
    comisionesStore.error = 'No se encontró la comisión a editar.'
    return
  }

  const coinA = editingForm.value.coin_a.trim().toUpperCase()
  const coinB = editingForm.value.coin_b.trim().toUpperCase()
  const percentage = Number(editingForm.value.percentage.trim())
  const reverse = editingForm.value.reverse.trim()
  const minAmount = Number(editingForm.value.min_amount.trim()) || 0
  const maxAmount = Number(editingForm.value.max_amount.trim()) || 0

  if (!coinA || !coinB) {
    comisionesStore.error = 'Monedas inválidas para la comisión.'
    return
  }

  if (percentage < 0 || Number.isNaN(percentage)) {
    comisionesStore.error = 'El porcentaje debe ser un número válido.'
    return
  }
  ...
  await comisionesStore.updateCommission(id, body)
  if (!comisionesStore.error) cancelEditing()
}
