      ...
      <table class="w-full text-left text-sm">
        ...
        <tbody>
          <tr
            v-for="user in users"
            :key="user.id"
            class="cursor-pointer border-t border-[#e5e7eb] bg-white transition hover:bg-[#f9fafb]"
            :class="
              selectedUserId === user.id
                ? 'bg-[#eef2ff] ring-1 ring-inset ring-brasper-indigoStrong/30'
                : ''
            "
          ...
          >
            ...
            <td class="px-4 py-3 text-[#374151]">{{ user.email || '-' }}</td>
            <td class="px-4 py-3 text-[#374151]">
              <span class="block text-xs text-[#6b7280]">{{ documentType(user) }}</span
              >{{ documentNumber(user) }}
            </td>
            <td class="px-4 py-3">
              <span
                class="inline-flex rounded-full bg-[#dbeafe] px-2.5 py-0.5 text-xs font-medium text-brasper-indigoDark"
                >{{
            ...
            <td class="relative px-2 py-3">
              <button
                ...
                @click.stop="emit('toggleMenu', user.id)"
              >
                •••
              </button>
              <div
                v-if="openMenuId === user.id"
                class="absolute right-0 top-full z-20 mt-1 min-w-[180px] rounded-lg border border-[#e5e7eb] bg-white py-1 shadow-lg"
                @click.stop
              >
                <button
                  v-if="canUpdate"
                ...
                <button
                  v-if="canViewAccounts && isClient(user)"
                  type="button"
                  class="block w-full px-4 py-2 text-left text-sm hover:bg-[#f9fafb]"
                ...
                <button
                  v-if="canDelete"
                  type="button"
                  class="block w-full px-4 py-2 text-left text-sm text-[#dc3545] hover:bg-[#fef2f2]"
    ...
    >
      ...
      <div class="flex items-center gap-2 text-sm text-[#6b7280]">
        ...
        <div class="flex gap-1">
          <button
            type="button"
            class="rounded p-2 hover:bg-[#f3f4f6] disabled:opacity-40"
