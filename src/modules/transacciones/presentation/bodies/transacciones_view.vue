...
} from '../composables/use_resizable_table_columns'
...
const { ensurePreviewCatalogLoaded, buildPreviewSections, buildPreviewDestinations } =
...
interface TransactionTableColumn<Key extends string = string> extends ResizableTableColumn<Key> {
  label: string
  headerClass: string
  title?: string
...
const TRANSACTION_TABLE_ACTIONS_WIDTH = 48
type TransactionTableColumnKey =
  | 'dailyNumber'
  | 'checked'
  | 'code'
  | 'operationNumber'
...
const TRANSACTION_TABLE_COLUMNS: readonly TransactionTableColumn<TransactionTableColumnKey>[] = [
  {
    key: 'dailyNumber',
    label: 'Número del día',
    defaultWidth: 56,
    minWidth: 44,
    maxWidth: 100,
    headerClass: 'px-3 py-3 text-right font-semibold text-brasper-indigoDark',
...
const {
  ...
  activeColumnKey: activeTransactionColumnKey,
  columnStyles: transactionColumnStyles,
  tableStyle: transactionTableStyle,
  startResize: startTransactionColumnResize,
  resizeBy: resizeTransactionColumnBy
} = useResizableTableColumns({
  columns: TRANSACTION_TABLE_COLUMNS,
  storageKey: 'brasper:transactions:table-column-widths:v1',
  fixedWidth: TRANSACTION_TABLE_ACTIONS_WIDTH
})
...
async function exportTransactionsToExcel() {
  ...
  try {
    ...
    const sheet = XLSX.utils.aoa_to_sheet([[...TRANSACTION_EXPORT_HEADERS], ...data])
    const book = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(book, sheet, 'Transacciones')
    XLSX.writeFile(
      book,
      transactionExportFilename(transactionScope.value, selectedDay.value, todayDayKey())
    )
  } catch (e) {
    exportError.value = e instanceof Error ? e.message : 'No se pudo generar el Excel'
  } finally {
...
const editableUsers = ref<{ id: string; name: string; email: string; role?: string }[]>([])
...
/** Impide que el GET de detalle tardío sobrescriba cambios ya hechos en el modal. */
const editFormRevision = shallowRef(0)
...
const destinationDraftsModel = computed({
  ...
  set: (value: TransactionDestinationDraft[]) => {
    ...
    form.bank_account_destination_id = form.destinations[0]?.bank_account_id?.trim() ?? ''
  }
})
...
const isLastCreateStep = computed(() => createStepIndex.value === CREATE_FLOW_STEPS.length - 1)
...
function goCreateNext() {
  ...
  if (i === 0) {
    ...
    if (
      ...
      transactionsStore.error = 'Completa la cotización: montos y tasa/comisión antes de continuar.'
      return
    }
    ...
    form.destination_amount = roundMoneyAmount(Number(form.destination_amount) || 0)
...
function openBankAccountModalFromTransaction(flow: 'origin' | 'destination') {
  if (!form.user_id?.trim()) {
    transactionsStore.error = 'Selecciona un cliente antes de crear una cuenta bancaria.'
    return
  }
  transactionsStore.error = null
...
function findBankAccountById(accountId: string): BankAccount | undefined {
  ...
  const userId = form.user_id?.trim()
  const fromClient = bankAccountsForSelectedClient().find((a) => String(a.id).trim() === id)
  if (fromClient) return fromClient
  const fromGlobal = cuentasStore.bankAccounts.find((a) => String(a.id).trim() === id)
  if (fromGlobal && userId && bankAccountBelongsToClient(fromGlobal, userId)) {
    return fromGlobal
  }
  const fromTxn = cuentasStore.transactionFormBankAccounts.find((a) => String(a.id).trim() === id)
...
function bankMetaFromCatalogBankId(bankId: string): {
  bank_id: string
  bank_name: string
  company_name: string
  ...
  const company_name = (bank.company ?? '').toString().trim() || bank_name || trimmed
  return {
    bank_id: trimmed,
    bank_name,
    company_name,
 ...
 */
function resolveStoredSocialReasonBankId(persistedBankId: string, companyName: string): string {
...
function bankMetaFromDestinationAccount(
  ...
  const holderLine = [account.holder_names, account.holder_surnames]
    ...
    .trim()
  const company_name =
    (isLegal ? (account.business_name ?? '').trim() : holderLine) || bank_name || ''
  return {
...
function resolveTransactionBankMeta(): {
  ...
  const fromAccount = bankMetaFromDestinationAccount(form.bank_account_destination_id)
  const razonSocial = bankMetaFromCatalogBankId(destinationBankFilterId.value)
  const socialReasonBankId = (
    destinationBankFilterId.value || (editingId.value ? editStoredSocialReasonBankId.value : '')
  ).trim()
  ...
  const razonSocialCompany = (
    razonSocial?.company_name ?? (editingId.value ? editStoredCompanyName.value : '')
  ).trim()
  if (fromAccount) {
    return {
      ...fromAccount,
...
const selectedAccountCurrencies = computed(() => {
  const selectedRate = tasasStore.taxRates.find((item) => item.id === form.tax_rate_id)
  return {
    origin: normalizeCurrencyCode(selectedRate?.coin_a ?? calculatorStore.currencyFrom),
    destination: normalizeCurrencyCode(selectedRate?.coin_b ?? calculatorStore.currencyTo)
  }
})
...
const currencyResolutionLookups = computed(() => ({
  ...
  commissionById: (id: string) => comisionesStore.commissions.find((item) => item.id === id),
...
function getTransactionCurrencies(t: Transaction, fallbackToSelected = false) {
  const pair = resolveTransactionCurrencyPair(t, currencyResolutionLookups.value)
  return {
    origin: pair.origin || (fallbackToSelected ? selectedAccountCurrencies.value.origin : ''),
    destination:
      pair.destination || (fallbackToSelected ? selectedAccountCurrencies.value.destination : '')
  }
}
...
function bankAccountMatchesCurrency(account: BankAccount, expectedCurrency: string): boolean {
...
function bankAccountMatchesFlow(account: BankAccount, flow: 'origin' | 'destination'): boolean {
  const accountFlow = (account.account_flow ?? '').trim().toLowerCase()
  if (!accountFlow) return true
  return accountFlow === flow
}

function bankAccountMatchesSide(account: BankAccount, flow: 'origin' | 'destination'): boolean {
...
function bankAccountsBelongingToClient(userId: string, accounts: BankAccount[]): BankAccount[] {
...
function bankAccountsForSelectedClient(): BankAccount[] {
  ...
  if (loadedForUser) {
    return bankAccountsBelongingToClient(userId, cuentasStore.transactionFormBankAccounts)
  }
...
const destinationAccountsLoading = computed(
  () => Boolean(form.user_id?.trim()) && cuentasStore.transactionFormBankAccountsLoading
)
...
function onBancoCrudSaved(payload?: { selectBankId?: string; deletedBankId?: string }) {
...
const clientOptions = computed(() => {
  ...
  const base = Array.from(byId.values()).sort((a, b) => a.label.localeCompare(b.label, 'es'))
  return mergeMissingTransactionUser(base, form.user_id)
})

function isSalesAdvisorRole(role?: string | null): boolean {
  return ['sales', 'ventas', 'commercial', 'comercial', 'advisor', 'asesor'].includes(
...
function getTransactionUserRoleLabel(role?: string | null): string {
  ...
  if (['commercial', 'comercial', 'advisor', 'asesor'].includes(normalized)) return 'Asesor'
...
function getCurrentUserEditableOption(): {
  ...
  return {
    id: user.id,
    name: user.name || [user.names, user.lastnames].filter(Boolean).join(' ') || user.email,
    email: user.email,
...
const taxRateOptions = computed(() => {
  ...
  const filtered =
    from && to
      ? all.filter(
          (r) => (r.coin_a ?? '').toLowerCase() === from && (r.coin_b ?? '').toLowerCase() === to
        )
      : []
...
const commissionOptions = computed(() => {
  ...
  const filtered =
    from && to
      ? all.filter(
          (c) => (c.coin_a ?? '').toLowerCase() === from && (c.coin_b ?? '').toLowerCase() === to
        )
      : []
  const list = filtered.length > 0 ? filtered : all
  return list.map((c) => ({
...
const showDestinationBankCatalogWarning = computed(
  () =>
    Boolean(destinationBankFilterId.value?.trim() || form.bank_account_destination_id?.trim()) &&
...
function getCalculatorBlockingError(): string | null {
  ...
  if (calculatorStore.calculationMode === 'special' && !res.specialDiscountValid) {
    return (
...
function syncCalculatorFromForm() {
  calculatorStore.resetCalculatorMode()
  const rate = calculatorStore.taxRates.find((item) => item.id === form.tax_rate_id)
  if (rate) {
  ...
  if (isEditingSpecialTransaction() && originAmount > 0 && destinationAmount > 0) {
...
function syncFromCalculator() {
  const res = calculatorStore.result
  form.origin_amount = roundMoneyAmount(res?.amountSend ?? calculatorStore.amountSend ?? 0)
  ...
  if (res) {
    const isSpecialCalculation =
      calculatorStore.calculationMode === 'special' || res.calculationMode === 'special'
...
function buildCreateQuoteSnapshot(): CreateQuoteSnapshot | null {
  ...
  return {
    ...
    origin_amount: roundMoneyAmount(isSpecial ? res.amountSend : form.origin_amount),
    destination_amount: roundMoneyAmount(isSpecial ? res.amountReceive : form.destination_amount),
    resultado_comision: roundMoneyAmount(
      isSpecial ? res.finalCommission : (form.resultado_comision ?? res.commission)
    ),
...
function getTransactionSpecialDiscount(t: Transaction): TransactionSpecialDiscountInfo | null {
  ...
  if (meta) {
    return {
      ...
      baseCommission: roundMoneyAmount(meta.discountCommission + Number(meta.finalCommission ?? 0)),
      finalCommission: roundMoneyAmount(Number(meta.finalCommission ?? 0)),
      totalToSend: meta.totalToSend,
      persisted: true
    }
  }
  return getTransactionSpecialDiscountForDisplay(t, getSpecialDiscountCatalogLookup())
}
...
async function loadEditableUsers() {
  ...
  editableUsers.value = Array.from(byId.values()).sort((a, b) => a.name.localeCompare(b.name, 'es'))
  ensureCurrentSalesAdvisorOption()
...
async function hydrateEditForm(row: Transaction) {
  ...
  try {
    ...
    form.bank_account_origin_id = normalizeSelectId(
      row.bank_account_origin_id ?? (row as Record<string, unknown>).bank_account_origin
    )
    form.bank_account_destination_id =
      normalizeSelectId(
        row.bank_account_destination_id ?? (row as Record<string, unknown>).bank_account_destination
    ...
    form.destination_amount = roundMoneyAmount(Number(row.destination_amount) || 0)
    {
      ...
      form.resultado_comision = rc != null && rc !== '' ? Number(rc) || null : null
    ...
    if (!socialReasonSelectionTouched.value) {
      ...
      editHasStoredSocialReasonBankIdField.value = hasStoredSocialReasonBankIdField
...
async function openEditModal(t: Transaction) {
  ...
  try {
    ...
    const revisionAfterInitialHydration = editFormRevision.value
    ...
    void (async () => {
      try {
        ...
        if (
          fresh &&
          editingId.value === transactionId &&
          editFormRevision.value === revisionAfterInitialHydration
        ) {
          ...
          await hydrateEditForm(enrichTransactionWithSpecialDiscountMeta({ ...t, ...fresh }))
          syncCalculatorFromForm()
        }
      } catch {
    ...
    transactionsStore.error = 'No se pudo preparar el formulario de edición. Intenta nuevamente.'
  } finally {
    editModalLoading.value = false
  }
}
...
function destinationsChangedForUpdate(): boolean {
  ...
  if (stored.length <= 1 && current.length === 1) {
    const storedId = stored[0]?.id || normalizeSelectId(source?.bank_account_destination_id)
    return current[0]?.id !== storedId
  }
  if (stored.length !== current.length) return true
  return stored.some(
    (row, index) => row.id !== current[index]?.id || row.amount !== current[index]?.amount
  )
}
...
async function submitForm() {
  ...
  try {
    ...
    const commonAmounts = {
      ...
      // En edición, solo un administrador puede reasignar el asesor.
      agent_id: editingId.value
        ? canEditTransactionAgent.value && form.agent_id?.trim()
          ? form.agent_id.trim()
          : undefined
        : form.agent_id?.trim() && form.agent_id.trim() !== form.user_id?.trim()
          ? form.agent_id.trim()
          : undefined,
      ...
      origin_amount: roundMoneyAmount(specialCreateSnapshot?.origin_amount ?? form.origin_amount),
      destination_amount: roundMoneyAmount(
        specialCreateSnapshot?.destination_amount ?? form.destination_amount
      ),
      resultado_comision:
        (specialCreateSnapshot?.resultado_comision ?? form.resultado_comision) != null
          ? roundMoneyAmount(specialCreateSnapshot?.resultado_comision ?? form.resultado_comision!)
          : undefined,
      total_a_enviar:
        (specialCreateSnapshot?.total_a_enviar ?? form.total_a_enviar) != null
          ? roundMoneyAmount(specialCreateSnapshot?.total_a_enviar ?? form.total_a_enviar!)
          : undefined,
      tax_amount:
      ...
      coupon_id: undefined,
      coupon_discount_code: specialCreateSnapshot ? SPECIAL_CALCULATOR_DISCOUNT_CODE : undefined,
      coupon_origin_amount: specialCreateSnapshot ? specialCreateSnapshot.origin_amount : undefined,
    ...
    if (editingId.value) {
      ...
      const updated = await transactionsStore.updateTransaction(editingId.value, {
        ...commonAmounts,
        ...voucherKeepPayload,
        destinations: destinationsChangedForUpdate() ? commonAmounts.destinations : undefined,
      ...
      const idx = transactionsStore.transactions.findIndex((row) => row.id === updated.id)
      if (idx >= 0) {
        transactionsStore.transactions[idx] = enrichTransactionWithSpecialDiscountMeta(updated)
      }
    } else {
      ...
      if (specialCreateSnapshot && created.id) {
        ...
        if (meta) {
          ...
          const idx = transactionsStore.transactions.findIndex((row) => row.id === created.id)
          if (idx >= 0) {
...
function updateMenuPosition() {
  ...
  if (point) {
    let px = point.x
    let py = point.y
    if (px + menuWidth > window.innerWidth - padding) px = window.innerWidth - menuWidth - padding
    if (py + menuHeight > window.innerHeight - padding) py = py - menuHeight
...
const previewVoucherTotal = computed(() =>
  previewVoucherGroups.value.reduce((sum, group) => sum + group.entries.length, 0)
)
...
const editPreviewTransaction = computed<Transaction | null>(() => {
  ...
  return {
    ...
    bank_account_origin_id: form.bank_account_origin_id || base.bank_account_origin_id,
    bank_account_destination_id:
...
function getCommissionPreviewLabel(id: string | undefined): string {
  ...
  return commission ? `${commission.coin_a}-${commission.coin_b} (${commission.percentage}%)` : id
}
...
const editHeroAmounts = computed(() => {
  ...
  const hasCoupon = Boolean(
    !specialDiscount && (t.coupon_id || (t.coupon_discount_code && t.coupon_discount_code.trim()))
  )
  const items = [
    {
    ...
    },
    {
      label: specialDiscount
        ...
        : hasCoupon
          ? 'Monto destino (Base)'
          : 'Monto destino',
      value: formatValueWithCurrency(t.destination_amount, currencies.destination)
    },
    {
      label: specialDiscount
        ? 'Comisión (Especial)'
        : hasCoupon
          ? 'Comisión (Base)'
          : 'Resultado comisión',
      value: formatValueWithCurrency(t.resultado_comision ?? t.commission_result, currencies.origin)
    }
  ]
...
function toggleMenu(id: string, event?: Event) {
  ...
  menuAnchorPoint.value = null
  menuTriggerEl.value =
    (event?.target as HTMLElement)?.closest('td') ?? (event?.target as HTMLElement) ?? null
  openMenuId.value = id
...
function transactionVoucherValues(
  ...
  return voucherValues([rec[field], rec[`${field}s`], rec[`${field}_files`]]).filter((value) => {
...
function persistedVoucherStrings(field: VoucherField): string[] {
  return persistedVoucherValues(field).filter((value): value is string => typeof value === 'string')
}
...
function removeDisplayedVoucher(field: VoucherField, index: number) {
  ...
  if (index < persistedCount) {
    ...
    if (typeof value === 'string') {
      removedPersistedVouchers[field] = [...removedPersistedVouchers[field], value]
      syncStatusFromVoucherFiles()
    }
    return
  }
...
function isFileValue(v: unknown): v is File {
  return typeof File !== 'undefined' && v instanceof File && typeof v.name === 'string'
}
...
const activeSendVoucherPreviewSrc = computed(() => {
  const entry = voucherDisplayEntries('send_voucher').find((item) => item.thumbSrc)
  return entry?.thumbSrc ?? null
})

/** Imagen de verificación (checklist) ya elegida o cargada en el formulario */
const isCheckedImageVoucherPresent = computed(() => hasVoucherValue(form.checked_image))
...
function onVoucherFileSelect(field: VoucherField, event: Event) {
...
function syncStatusFromVoucherFiles() {
  ...
  } else if (hasCheckedImage && (currentStatus === 'verification' || currentStatus === '')) {
    form.status = 'verified'
  }
}
...
async function submitImportSimple() {
  ...
  transactionsStore.error = null
  try {
    await Promise.all([cuentasStore.loadBankAccounts(), cuentasStore.loadBanks()])
...
watch(
  form,
  () => {
    if (isEditingMode.value && !isHydratingTransactionForm.value) {
      editFormRevision.value += 1
    }
  },
...
watch(
  ...
  (userId) => {
    ...
    form.bank_account_origin_id = ''
    form.bank_account_destination_id = ''
    form.destinations = clearTransactionDestinationAccounts(form.destinations)
    if (!isEditingMode.value) {
...
<template>
  <div class="space-y-6">
      ...
      >
        <div class="flex flex-wrap items-center gap-3">
          ...
          <p v-else class="text-sm text-[#374151]">
            Histórico completo · <strong>{{ totalResults }}</strong> envíos. La columna
            <code class="rounded bg-white px-1">#</code> sigue siendo el número del envío dentro de
            su día.
          </p>
        </div>
    ...
    >
      ...
      <table class="table-fixed text-left text-sm" :style="transactionTableStyle">
        <colgroup>
          <col
            v-for="column in TRANSACTION_TABLE_COLUMNS"
            :key="column.key"
            :style="transactionColumnStyles[column.key]"
          />
          <col :style="{ width: `${TRANSACTION_TABLE_ACTIONS_WIDTH}px` }" />
        </colgroup>
        <thead>
          <tr class="bg-[#dbeafe]">
            <th
            ...
            >
              ...
              <TableColumnResizeHandle
                :label="column.label"
                :width="transactionColumnWidths[column.key]"
                :min-width="column.minWidth"
                :max-width="column.maxWidth"
                :active="activeTransactionColumnKey === column.key"
                @resize-start="startTransactionColumnResize(column.key, $event)"
                @resize-step="resizeTransactionColumnBy(column.key, $event)"
              />
            </th>
            <th
              class="sticky right-0 z-[2] bg-[#dbeafe] px-2 py-3 shadow-[-8px_0_12px_-12px_rgba(15,23,42,0.45)]"
            ></th>
          </tr>
        </thead>
        <tbody>
          <tr v-if="paginatedTransactions.length === 0" class="border-t border-[#e5e7eb]">
            ...
            >
              <template
                v-if="
                  transactionsStore.hasLoadedOnce && totalResults === 0 && !transactionsStore.error
                "
              >
                <p class="mb-3">No hay transacciones que coincidan con los filtros.</p>
                <button
                  type="button"
              ...
              <template v-else> No hay transacciones en esta página. </template>
            </td>
          </tr>
          <tr
          ...
          >
            <td
              class="overflow-hidden whitespace-nowrap px-3 py-3 text-right text-xs font-medium tabular-nums text-[#9ca3af]"
            >
            ...
            <td class="overflow-hidden px-4 py-3 text-[#374151]">
              ...
              <span
                ...
                class="mt-1 flex max-w-full flex-wrap gap-1 overflow-hidden"
              >
                <span
                  v-for="tag in transactionTags(t)"
                  :key="tag.id"
                  class="inline-flex max-w-full items-center truncate rounded-full border px-2 py-px text-[10px] font-semibold"
            ...
            <template v-else-if="previewTransaction">
              ...
              >
                <div class="flex flex-wrap items-center gap-x-6 gap-y-3">
                  <div>
                    <p class="text-[10px] font-semibold uppercase tracking-widest text-[#9ca3af]">
                      Código
                    </p>
                    <p class="font-mono text-2xl font-bold text-brasper-indigoStrong leading-tight">
                      {{
                        previewSectionGroups.resumen.items.find((i) => i.label === 'Código')
                          ?.value ?? '—'
                      }}
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <span
                      ...
                      :class="
                        statusRowBadgeClass(
                          resolveTransactionStatusForDisplay(previewTransaction) ??
                            previewTransaction.status
                        )
                      "
                    >
                      {{
                        previewSectionGroups.resumen.items.find((i) => i.label === 'Estado')
                          ?.value ?? '—'
                      }}
                    </span>
                  </div>
                  ...
                  <div>
                    ...
                    <span
                      ...
                      :class="
                        previewSectionGroups.resumen.items.find((i) => i.label === 'Verificada')
                          ?.value === 'Sí'
                  ...
                  <div>
                    ...
                    <p class="mt-1 text-sm font-semibold text-[#111827]">
                      {{
                        previewSectionGroups.resumen.items.find(
                          (i) => i.label === 'Ventas / asesor'
                        )?.value ?? '—'
                      }}
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <p class="mt-1 font-mono text-sm font-semibold tabular-nums text-[#111827]">
                      {{
                        previewSectionGroups.resumen.items.find((i) => i.label === 'N.º operación')
                          ?.value ?? '—'
                      }}
                    </p>
                  </div>
              ...
              <div class="grid grid-cols-1 lg:grid-cols-2 lg:divide-x lg:divide-[#e8eef8]">
                ...
                <div class="space-y-4 p-5">
                  <section
                    v-for="section in previewSectionGroups.left.filter(
                      (s) => s.id === 'participantes'
                    )"
                    :key="section.id"
                  ...
                  >
                    ...
                    <dl class="space-y-2.5">
                      ...
                      >
                        <dt
                          class="shrink-0 text-xs font-medium uppercase tracking-wide text-[#6b7280] sm:min-w-[8rem]"
                        >
                          {{ item.label }}
                        </dt>
                  ...
                  >
                    <div
                      class="mb-3 flex items-start justify-between gap-3 border-b border-[#e5e7eb]/80 pb-2"
                    >
                      <div>
                        <h3
                          class="text-[11px] font-semibold uppercase tracking-[0.14em] text-brasper-indigoStrong"
                        ...
                        <p class="mt-1 text-xs text-[#6b7280]">Distribución del monto a recibir</p>
                      </div>
                      <span
                        class="shrink-0 rounded-full bg-[#eef2ff] px-2.5 py-0.5 text-[11px] font-semibold tabular-nums text-brasper-indigoStrong"
                      >
                      ...
                      >
                        <div class="flex items-start justify-between gap-3">
                          <div class="min-w-0">
                            ...
                            <div v-if="row.identifiers.length" class="mt-1 space-y-1">
                              <button
                              ...
                              >
                                ...
                                <span
                                  class="shrink-0 font-sans text-[10px] font-semibold text-brasper-indigoStrong"
                                >
                                  {{
                                    copiedPreviewAccountIdentifier === identifier.value
                                      ? 'Copiado'
                                      : 'Copiar'
                                  }}
                                </span>
                ...
                <div class="space-y-4 border-t border-[#e8eef8] p-5 lg:border-t-0">
                  ...
                  >
                    ...
                    <dl class="space-y-2.5">
                      ...
                      >
                        <div
                          v-if="item.variant === 'separator'"
                          class="-mx-1 my-1 border-t border-[#d8e5fb]"
                        />
                        <div v-else class="flex items-baseline justify-between gap-4">
                          <dt class="text-xs font-medium text-[#6b7280]">{{ item.label }}</dt>
                          <dd class="tabular-nums text-sm font-semibold text-[#111827]">
                            {{ item.value }}
                          </dd>
                        </div>
                      </template>
              ...
              >
                ...
                >
                  ...
                  <dl class="grid grid-cols-1 gap-x-8 gap-y-2.5 sm:grid-cols-2">
                    ...
                    >
                      <dt class="text-[10px] font-semibold uppercase tracking-wide text-[#9ca3af]">
                        {{ item.label }}
                      </dt>
                      <dd class="text-xs font-medium text-[#374151]">
                        <span v-if="item.variant === 'mono'" class="break-all font-mono">{{
                          item.value
                        }}</span>
              ...
              >
                <div class="mb-3 flex flex-wrap items-baseline justify-between gap-2">
                  ...
                  <span
                    class="rounded-full bg-[#eef2ff] px-2.5 py-1 text-[11px] font-semibold tabular-nums text-brasper-indigoStrong"
                  >
                    {{ previewVoucherTotal }}
                    {{ previewVoucherTotal === 1 ? 'archivo' : 'archivos' }}
                  </span>
                </div>
                <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  ...
                  >
                    <div
                      class="flex items-center justify-between gap-2 border-b border-[#e5e7eb] bg-white px-3 py-2"
                    >
                      <span class="text-xs font-semibold text-[#374151]">{{ group.title }}</span>
                      <span
                        class="rounded-full bg-[#f3f4f6] px-2 py-0.5 text-[10px] font-semibold tabular-nums text-[#6b7280]"
                      >
                    ...
                    <div class="flex-1 space-y-2 p-2">
                      ...
                      >
                        ...
                        <span class="flex items-center gap-2 px-3 py-2">
                          ...
                          <span
                            class="ml-auto shrink-0 text-[11px] font-semibold text-brasper-indigoStrong opacity-0 transition group-hover/voucher:opacity-100"
                          >
                            Abrir
                          </span>
                        </span>
    ...
    <Teleport to="body">
      ...
      >
        ...
        >
            ...
            <div class="min-w-0 flex-1">
              <div class="flex items-center gap-3">
                ...
                <div class="min-w-0">
                  ...
                  <p v-if="!isEditingMode" class="mt-0.5 text-xs font-medium text-[#6b7280]">
                    Paso {{ createStepIndex + 1 }} de {{ CREATE_FLOW_STEPS.length }} ·
                    {{ CREATE_FLOW_STEPS[createStepIndex]?.title ?? '' }}
                  </p>
                  <p v-else class="mt-0.5 text-sm leading-relaxed text-[#6b7280]">
              ...
              >
                <div class="flex w-full min-h-[2.75rem] items-center">
                  <div
                    ...
                    :class="createStepIndex >= idx ? 'bg-brasper-indigoStrong' : 'bg-[#e5e7eb]'"
                  />
                  <button
                    type="button"
          ...
          <!-- Contenido -->
          <div class="flex-1 overflow-y-auto" :class="isEditingMode ? '' : 'p-6'">
            <form v-if="isEditingMode" class="flex min-h-0 flex-1" @submit.prevent="submitForm">
              <div
              ...
              >
                ...
                />
                <div>
                  <h3 class="text-sm font-semibold text-[#1f2937]">Cargando transacción</h3>
                  <p class="mt-1 text-sm text-[#6b7280]">
                    Estamos preparando el formulario y los catálogos para editar.
                  </p>
                </div>
              </div>
              ...
              <template v-else>
                ...
                >
                  <div class="flex-1 overflow-y-auto p-5">
                    <div class="mx-auto max-w-2xl space-y-5">
                      ...
                      >
                        <p
                          class="text-[10px] font-semibold uppercase tracking-[0.22em] text-brasper-indigoStrong"
                        >
                          Cotización
                        </p>
                        <h3 class="mt-2 text-lg font-semibold text-[#1f2937]">Corregir montos</h3>
                        <p class="mt-1 text-sm leading-relaxed text-[#6b7280]">
                          Ajusta el monto en la calculadora y aplica los cambios para volver a la
                          edición.
                        </p>
                      </div>
                ...
                >
                  <div class="hidden min-h-0 flex-col gap-4 text-[#1f2937] xl:flex">
                    <div
                      class="grid grid-cols-2 gap-3 rounded-2xl border border-[#e6ebf4] bg-white p-4 sm:grid-cols-4"
                    >
                      <div class="min-w-0">
                        <span
                          class="block text-[10px] font-semibold uppercase tracking-[0.2em] text-[#7b88a1]"
                        >
                          Código
                        </span>
                    ...
                    <div class="space-y-4">
                      <section class="rounded-2xl border border-[#e6ebf4] bg-white p-4">
                        <div class="flex items-center gap-2">
                          ...
                          <h3
                            class="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#50607c]"
                          >
                            Participantes
                          </h3>
                        </div>
                        <dl class="mt-4 space-y-3.5">
                          ...
                          >
                            <dt
                              class="text-[11px] font-semibold uppercase tracking-[0.14em] text-[#7b88a1]"
                            >
                              {{ item.label }}
                            </dt>
                            <dd
                              class="max-w-[13rem] text-right text-sm font-semibold leading-relaxed text-[#1f2937]"
                            >
                              {{ item.value }}
                            </dd>
                          </div>
                        ...
                        <div class="mt-4 rounded-3xl border border-[#e8eef8] bg-[#fbfdff] p-4">
                          <dl class="space-y-3">
                            ...
                            >
                              ...
                              <dd
                                class="text-right text-[1.35rem] font-semibold tracking-[0.01em] text-[#1f2937]"
                              >
                                {{ item.value }}
                              </dd>
                            </div>
                      ...
                      <div class="space-y-5">
                        <section class="rounded-2xl border border-[#edf1f6] bg-[#fcfdff] p-5">
                          <div class="mb-4 flex items-center justify-between gap-3">
                            <div>
                              ...
                              <p class="mt-1 text-xs text-[#6b7280]">Usuario, cuentas y fechas.</p>
                            </div>
                          </div>
                          <div class="grid gap-4 sm:grid-cols-2">
                            ...
                            <div class="space-y-1.5">
                              ...
                              <div
                                v-else
                                class="flex h-10 w-full items-center rounded-lg border border-[#e5e7eb] bg-[#f3f4f6] px-3 text-sm text-[#6b7280]"
                                aria-readonly="true"
                              >
                                {{ editModalSummary?.agent ?? '—' }}
                              </div>
                            ...
                            <div
                              class="grid gap-4 sm:col-span-2 sm:grid-cols-[minmax(0,1fr)_minmax(220px,280px)] sm:items-start"
                            >
                              <div class="space-y-1.5">
                                <label class="block text-sm font-medium text-[#374151]"
                        ...
                        <section class="rounded-2xl border border-[#edf1f6] bg-[#fcfdff] p-5">
                          <div class="mb-4">
                            ...
                            <p class="mt-1 text-xs text-[#6b7280]">
                              Los archivos ya subidos se conservan al guardar; «Quitar» los elimina
                              y puedes agregar nuevos.
                            </p>
                          </div>
                          <div class="grid gap-4 md:grid-cols-3">
                            ...
                            >
                              <div class="mb-3 flex items-start justify-between gap-3">
                                <div>
                                  <h4 class="text-sm font-semibold text-[#232b4d]">Envío</h4>
                              ...
                              <TransactionVoucherFileList
                                ...
                                @preview="
                                  openMediaViewer($event.href || $event.thumbSrc, $event.label)
                                "
                              />
                            </div>
                            ...
                            >
                              <div class="mb-3 flex items-start justify-between gap-3">
                                <div>
                                  <h4 class="text-sm font-semibold text-[#232b4d]">Pago</h4>
                                  <p class="mt-0.5 text-xs text-[#6b7280]">Opcional</p>
                                </div>
                              </div>
                              <label
                            ...
                            >
                              <div class="mb-3 flex items-start justify-between gap-3">
                                <div>
                                  <h4 class="text-sm font-semibold text-[#232b4d]">Verificación</h4>
                                  <p class="mt-0.5 text-xs text-[#6b7280]">Marca la operación</p>
                                </div>
                              </div>
                              <label
                                class="inline-flex cursor-pointer items-center justify-center rounded-xl border border-[#d5def0] bg-[#f8faff] px-4 py-2.5 text-sm font-semibold text-brasper-indigoStrong transition hover:border-brasper-indigoStrong/40 hover:bg-white"
            ...
            <div v-else-if="createStepIndex === 0" class="min-w-0 space-y-5">
              <p
            ...
            >
              ...
              >
                ...
                <div class="grid gap-5 sm:grid-cols-2">
                  ...
                  <div class="space-y-1.5">
                    <label class="block text-sm font-medium text-[#374151]">Ventas / asesor</label>
                    <AppDropdown
                      v-model="form.agent_id"
                      :options="salesAdvisorOptions"
                      placeholder="Seleccionar asesor"
                  ...
                  <div
                    ...
                    class="sm:col-span-2 rounded-lg border border-amber-200 bg-amber-50/90 px-3 py-2 text-xs text-amber-950"
                  >
                    No se encontró el banco de esta cuenta en el catálogo local. Recarga la página o
                    revisa la cuenta en Cuentas bancarias.
                  </div>
                  <p
              ...
              >
                <div class="mb-4 flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p class="text-xs text-[#6b7280]">
                      Heredados del paso de cotización; editables si es necesario
                    </p>
                  </div>
                </div>
                <div class="grid gap-5 sm:grid-cols-2">
                  ...
                  <div class="space-y-1.5">
                    <label
                      class="flex items-center justify-between gap-2 text-sm font-medium text-[#374151]"
                    >
            ...
            <form v-else-if="createStepIndex === 2" class="space-y-5" @submit.prevent="submitForm">
              <p class="text-sm leading-relaxed text-[#6b7280]">
                Adjunta comprobantes (PDF o imagen). Son opcionales y se envían al guardar con el
                resto del alta.
              </p>

              <div
                v-if="
                  formVoucherValues('send_voucher').length &&
                  formVoucherValues('payment_voucher').length
                "
              ...
              >
                ...
                <span>
                  Tienes ambos comprobantes listos. Al guardar, el estado puede pasar a finalizado
                  si aplica.
                </span>
              </div>
              ...
              <div class="grid gap-5 md:grid-cols-3">
                ...
                >
                  <div class="mb-4 flex items-start gap-3">
                    ...
                    <div class="min-w-0">
                      <h3 class="text-base font-semibold text-[#232b4d]">Envío</h3>
                      <p class="mt-0.5 text-xs text-[#6b7280]">Archivo de envío</p>
                    </div>
                  </div>
                  <label
                ...
                >
                  <div class="mb-4 flex items-start gap-3">
                    ...
                    <div class="min-w-0">
                      <h3 class="text-base font-semibold text-[#232b4d]">Pago</h3>
                      <p class="mt-0.5 text-xs text-[#6b7280]">Archivo de pago</p>
                    </div>
                  </div>
                  <label
                    class="inline-flex cursor-pointer items-center justify-center rounded-lg border border-[#cfdbef] bg-[#fbfdff] px-4 py-2.5 text-sm font-semibold text-brasper-indigoStrong transition hover:border-brasper-indigoStrong/40 hover:bg-white"
                ...
                >
                  <div class="mb-4 flex items-start gap-3">
                    ...
                    <div class="min-w-0">
                      <h3 class="text-base font-semibold text-[#232b4d]">Verificación</h3>
                      <p class="mt-0.5 text-xs text-[#6b7280]">Imagen de checklist</p>
                    </div>
                  </div>
                  <label
                    class="inline-flex cursor-pointer items-center justify-center rounded-lg border border-[#cfdbef] bg-[#fbfdff] px-4 py-2.5 text-sm font-semibold text-brasper-indigoStrong transition hover:border-brasper-indigoStrong/40 hover:bg-white"
              ...
              <div
                class="space-y-1.5 rounded-xl border border-[#d8e5fb] bg-white p-5 shadow-sm shadow-brasper-indigoStrong/5"
              >
                <label class="block text-sm font-medium text-[#374151]">
                  Número de operación *
                </label>
      ...
      >
        <div class="w-full max-w-md rounded-2xl border border-[#dbe7fb] bg-white p-6 shadow-xl">
          ...
          <div class="mb-4 rounded-lg bg-[#fbfdff] p-4 text-sm text-[#666]">
            ...
            <p class="mb-2">
              Si el Excel tiene columnas <strong>Nombre</strong>, <strong>Correo</strong>,
              <strong>ENVÍA (PEN)</strong>, <strong>RECIBE (BRL)</strong>, etc., se detecta
              automáticamente.
            </p>
            ...
            <ul class="list-inside list-disc space-y-1">
              ...
              <li>
                <strong>tax_rate_id</strong>, <strong>commission_id</strong> — IDs de tasa y
                comisión
              </li>
              <li>
                <strong>origin_names</strong>, <strong>origin_lastnames</strong>,
              ...
              <li>
                <strong>origin_bank_id</strong>, <strong>dest_bank_id</strong> — IDs de bancos
              </li>
              <li>
                <strong>send_date</strong> / fecha_envio, <strong>payment_date</strong> / fecha_pago
              </li>
            </ul>
          </div>
      ...
      >
        <div class="w-full max-w-md rounded-2xl border border-[#e5e7eb] bg-white p-6 shadow-xl">
          ...
          <p class="mb-4 text-sm text-[#6b7280]">
            Usa el mismo endpoint que Crear. Cada fila del Excel se envía como una transacción
            individual.
          </p>
