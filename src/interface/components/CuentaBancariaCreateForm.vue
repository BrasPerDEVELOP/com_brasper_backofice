...
function submitCreate() {
  ...
  if (!userId) {
    emit('validationError', 'Seleccione un cliente')
    return
  }
  ...
  if (requireAccountNumber.value) {
    if (!accountNumber) {
      emit('validationError', 'Ingrese el número de cuenta')
      return
    }
    if (!accountNumberConfirmation) {
      emit('validationError', 'Confirme el número de cuenta')
      return
    }
    if (accountNumber !== accountNumberConfirmation) {
      emit('validationError', 'El número de cuenta y su confirmación no coinciden')
      return
    }
  } else if (
    accountNumber &&
        ...
        <div class="space-y-1.5">
          ...
          <div class="flex gap-2">
            <AppDropdown
              ...
              :options="bankOptions"
              :placeholder="banksLoading ? 'Cargando...' : 'Seleccionar banco'"
