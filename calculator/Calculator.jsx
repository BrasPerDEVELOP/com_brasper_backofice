...
const Calculator = ({ updateDetails }) => {
  ...
  const calculate = useCallback(
    (amount, isReceiveAmount = false) => {
      ...
      const rateValue = exchangeRates[key];
      ...
      if (isReceiveAmount) {
        ...
        while (iterations < 5) {
          ...
          const autoDisc = (() => {
            ...
            const applicable = autoDiscountRules.filter((r) =>
              r.minimum_amount == null ? true : estimateSend >= r.minimum_amount
            );
        ...
        const autoDiscFinal = (() => {
          if (!autoDiscountRules.length) return 0;
          const applicable = autoDiscountRules.filter((r) =>
            r.minimum_amount == null ? true : amountSendCalc >= r.minimum_amount
          );
          const maxPct = applicable.reduce(
            (m, r) => Math.max(m, Number(r.discount_percentage || 0)),
            0
        ...
        const autoDisc = (() => {
          if (!autoDiscountRules.length) return 0;
          const applicable = autoDiscountRules.filter((r) =>
            r.minimum_amount == null ? true : parsedAmount >= r.minimum_amount
          );
          const maxPct = applicable.reduce(
            (m, r) => Math.max(m, Number(r.discount_percentage || 0)),
            0
